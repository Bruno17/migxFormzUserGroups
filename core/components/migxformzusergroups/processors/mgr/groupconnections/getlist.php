...
$connections = array();
$c2 = $modx->newQuery('fmzFormsUsergroup');
$c2->where(array('form_id'=>$object_id));
if ($connectionsCollection = $modx->getCollection('fmzFormsUsergroup',$c2)){
    foreach ($connectionsCollection as $connectionsObject){
        $connections[] = $connectionsObject->get('group_id');
    }
}
...
if ($collection = $xpdo->getCollection($classname, $c)) {
    ...
    foreach ($collection as $object) {
        ...
        $row['joined'] = in_array($row['id'],$connections) ? '1' : '0';
        $rows[] = $row;
    }
}
